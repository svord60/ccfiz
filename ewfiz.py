...
ITEMS_PER_PAGE = 5  # Количество стран на странице
...
class AddAccount(StatesGroup):
    ...
    waiting_price = State()
    waiting_phone = State()
    waiting_password = State()  # Новое: облачный пароль
    waiting_code = State()
...
async def promo_code_check(message: Message, state: FSMContext):
    ...
    # Проверяем срок действия (5 минут после исчерпания)
